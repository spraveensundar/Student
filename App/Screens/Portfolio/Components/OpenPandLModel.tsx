...
import { useEditOpenOrdersMutation } from "../../../Slices/future";
...
const OpenPandLModel: React.FC<ProfitLossModalprops> = ({
    ref,
    orderData,
    refetch
}) => {
    ...
    const [editOpenOrder, { isLoading, error }] = useEditOpenOrdersMutation();
    ...
    const changeBraketOrder = async () => {
        ...
        const payload: any = {
            ...
            isTake: formdata.isTake,
            isStop: formdata.isStop
        }
        const response = await editOpenOrder(payload).unwrap();
        if (response.success) {
    ...
    const setGainAndLoss = (type: any, value: any, call = 'edit') => {
        try {
            ...
            if (orderData.positionSide == 'short') {
                marketPrice = isTake ? orderData.price < marketPrice ? orderData.price : marketPrice : orderData.price > marketPrice ? orderData.price : marketPrice
    ...
    useEffect(() => {
        ...
        if (isEmpty(updateData.takeProfit.stopPrice)) {
            ...
            console.log('setGainAndLoss', stopPrice)
        ...
        if (isEmpty(updateData.stopLoss.stopPrice)) {
            ...
            console.log('setGainAndLoss1', stopPrice)
            ...
            >
                <View style={{ paddingHorizontal: "5%", flex: 1, paddingBottom: "5%" }} >
                    <View style={{ flexDirection: "row", width: windowwidth, alignSelf: "center", height: windowheight * 0.08, paddingHorizontal: "5%", justifyContent: "space-between", alignItems: "center" }} >
                        ...
                        <Pressable style={{ backgroundColor: theme.card, borderRadius: 10, padding: 10 }} onPress={onClose}>
                            <VectorIcons
                                family="Ionicons"
                                name="close"
