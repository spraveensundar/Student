...
import { useLazyGetRideMessagesQuery } from "../../Common/redux/rideServiceHooks";
import { sendRideChatMessage } from "../../Common/axiosHooks/rideServiceHooks";
...
type Props = NativeStackScreenProps<Stacknavigationtypes, 'RideChatBox'>;
...
const RideChatBox: React.FC<Props> = ({ route }) => {



    const [trigger] = useLazyGetRideMessagesQuery();
    ...
    const rideId = route?.params?.rideDetail?._id, rideDetail = route?.params?.rideDetail, instanceId = route?.params?.rideDetail?.instanceId;

    console.log('serviceDetailserviceDetail', rideDetail)
    ...
    const sendMessage = async () => {
        ...
        let resp = await sendRideChatMessage(sendData);
        console.log('dsfsdfsd', resp,sendData);
        if (resp?.status) {
    ...
    const events: any = {
        [constantData.socketEvents.rideServiceChatNewMessage]: useCallback((data: any) => {
            console.log('dataaa', data, messages)
            if (data?.instanceId == rideDetail?.instanceId && rideDetail?.instanceId) {
    ...
    console.log('serviceDetail?.servicemanDetail', rideDetail?.servicemanDetail)

    const isOnline = () => {
        if (rideDetail?.driverDetail?.isOnline) {
            return true;
        }
        return false;
    }
        ...
        >
            <Flexcomponent paddingVertical={"5%"} >
                ...
                <View style={{ flexDirection: 'row', gap: 15, width: "80%" }}>
                    <Images
                        type="image"
                        source={rideDetail?.driverDetail?.profile ? { uri: returnOriginalFile(rideDetail?.driverDetail?.profile) } : icons.Man}
                    />
                    <View>
                        <Text family="GMedium" size="medium"
                        >
                            {
                                rideDetail?.driverId
                                    ?
                                    (rideDetail?.driverDetail?.firstName || "Driver")
                                    :
                                    "Driver not assigned"
                            }
                        </Text>
                        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                            {/* {
                                serviceDetail?.driverDetail?._id
                                    ?
                                    <>
                                        <View
                                            style={{
